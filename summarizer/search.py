...
import numpy as np
from pointer_model.seq2seqModel import Seq2SeqModel
from pointer_model.transformerModel import TransformerDecoder
from joeynmt.helpers import tile


def greedy(src_mask: Tensor, src_ids: Tensor, max_output_length: int, model: Seq2SeqModel,
    ...
    return greedy_fun(
        src_mask, src_ids, max_output_length, model, encoder_output, encoder_hidden)


def recurrent_greedy(
        src_mask: Tensor, src_ids: Tensor, max_output_length: int, model: Seq2SeqModel,
        encoder_output: Tensor, encoder_hidden: Tensor):
    """
    ...
    """
    bos_index = model.lexicon.tokenizer.cls_token_id
    eos_index = model.lexicon.tokenizer.eos_token_id
    batch_size = src_mask.size(0)
    prev_y = src_mask.new_full(size=[batch_size, 1], fill_value=bos_index,
                               dtype=torch.long)
    #device = model.lexicon.model.device
    output = []
    attention_scores = []
    hidden = None
    ...
    for t in range(max_output_length):
        ...
        with torch.no_grad():
            #prev_y = prev_y.to(model.lexicon.model.device)
            trg_embed, _ = model._embed_input_ids(ids=prev_y)
...
def transformer_greedy(
    ...
    for _ in range(max_output_length):
        ...
        with torch.no_grad():
            #prev_y = prev_y.to(model.lexicon.model.device)
            trg_embed, _ = model._embed_input_ids(ids=ys)
            logits, _, _, _ = model(
                encoder_output=encoder_output,
                encoder_hidden=None,
                src_mask=src_mask,
            ...
            ys = ys.to(model.lexicon.model.device)
...
def beam_search(model: Seq2SeqModel, size: int,
                encoder_output: Tensor, encoder_hidden: Tensor,
                src_ids: Tensor,
    ...
    bos_index = model.lexicon.tokenizer.cls_token_id
    eos_index = model.lexicon.tokenizer.eos_token_id
    pad_index = model.lexicon.tokenizer.pad_token_id
    ...
    # tile src ids for pointer decoder
    src_ids = tile(src_ids, size, dim=0)
    ...
    #print('batch offset size batch size ', batch_offset.size(), batch_offset)
    ...
    beam_offset = torch.arange(
        ...
        dtype=torch.long,
        device=device)
    #print('beam offset size batch size * beam size step size = beam size ', beam_offset.size(), beam_offset)
    ...
    for step in range(max_output_length):
        ...
        # pylint: disable=unused-variable
        print('current step and current decoder input length ', step, decoder_input.size())
        with torch.no_grad():
            #decoder_input = decoder_input.to(model.lexicon.model.device)
            trg_embed, _=model._embed_input_ids(ids=decoder_input) # trg_embed = embed(decoder_input)
            log_probs, hidden, att_scores, att_vectors = model(
                encoder_output=encoder_output,
                ...
                unroll_steps=1,
                trg_mask=trg_mask,
                generate_mode=True # subsequent mask for Transformer only
            )
        ...
        # batch*k x trg_vocab
        log_probs = log_probs[:, -1].to(device).squeeze(1)
        ...
        if hidden is not None and not transformer:
            ...
            #    hidden = (h, c)
                ...
                hidden = hidden.to(device).index_select(0, select_indices)

        if att_vectors is not None:
            att_vectors = att_vectors.to(device).index_select(0, select_indices)
...
def run_batch(model: Seq2SeqModel, batch: List, max_output_length: int,
    ...
    """
    with torch.no_grad():
        xs = [s[0][:2]+s[1] for s in batch]
        src_batch, src_lens, original_idx_src = model.lexicon.encode_batch(xs)
        encoder_output, encoder_hidden= model._encode(src_batch, src_lens)

    # if maximum output length is not globally specified, adapt to src len
    if max_output_length is None:
        max_output_length = int(src_lens[0]/4)
        print('max output length in current batch: ', max_output_length)

    # greedy decoding
    src_mask = src_batch['attention_mask']
    src_ids = src_batch['input_ids']
    if beam_size < 2:
